...
public class TextChannelParameterRequiresGuildCoCo implements CommandASTCommandCoCo, CommandInheritanceVisitor {
    public static final String ERROR_MESSAGE = "Only guild commands can have text channel parameters.";

    protected boolean inGuild;
    protected boolean hasTextChannel;
    ...
    public void visit(ASTParameterVariable node){
        if(node.getParameter() == ASTParameter.TEXT_CHANNEL)
            hasTextChannel = true;
    }
}
